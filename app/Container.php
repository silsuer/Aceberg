...
// 依赖注入容器类，每一次请求进来的时候，都把需要的东西注入到容器中，需要的时候，从容器中取出即可
class Container
{
    /**
    ...
    public function get($class, $params = array(), $properties = array())
    {
        ...
        if (is_callable($defination, true)) {//如果声明是函数
            ...
            $obj = call_user_func($defination, $this, $params, $properties);  // 调用函数
    ...
    {
        // 如果这个类依赖的参数为空的话，直接返回参数
        if (empty($this->_params[$class])) {
            return $params;
        }
        // 如果传入的参数为空的话，返回整个参数列表
        if (empty($params)) {
            return $this->_params;
        }
    ...
    protected function getDependencies($class)
    {
        ...
        $dependencies = array();
        $ref = new \ReflectionClass($class);// 通过反射 获取对象的实例
        ...
        if ($constructor !== null) {//如果构造方法有参数
            foreach ($constructor->getParameters() as $param) {//获取构造方法的参数
                if ($param->isDefaultValueAvailable()) {//如果是默认 直接取默认值
                    ...
                    $temp = $param->getClass();
                    $temp = ($temp === null ? null : $temp->getName());  // 获取类名
